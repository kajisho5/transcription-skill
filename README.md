...
Requirements: Python 3.9+, FFmpeg (`ffmpeg` and `ffprobe` on PATH), and an ASR engine. The reference
engine is [faster-whisper](https://github.com/SYSTRAN/faster-whisper), an optional dependency that
runs Whisper locally on CPU or GPU. The `base` model (~145 MB) is downloaded into the Hugging Face
cache on first use; copy it there yourself for offline machines. No API keys, no cloud calls.
...
transcription skill --json                             # the skill / tool contract
```
...
```json
{
  ...
  "provenance": {"model": "base", "model_version": "ebe41f70…", "parameters": {"language": "ja", "beam_size": 5, "temperature": 0.0, "word_timestamps": true, "initial_prompt": null},
                 "parameters_hash": "6edc…", "cache_key": "301d…", "processing_seconds": 3.3, "skill_version": "0.1.0"},
  "warnings": []
}
```
...
generation, subtitle styling, video editing, plugin loading, and arbitrary command execution.
...
and tools. This skill exposes exactly that through `transcription skill --json`. An adapter on the
agent's side would call `transcription/transcribe` as a process (`--json` on stdout, the same contract
ffmpeg-skill uses), store the Transcript as an Observation, and lift `transcription/segments` output
into its `Event` model (`type`, `range`, `source`, `kind=OBSERVED`, `confidence`, `evidence`,
`metadata`). The SpeechEvent record here mirrors that shape without importing it; the agent is not a
dependency of this repository and no adapter is added to the agent in this version.
